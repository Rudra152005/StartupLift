...
import { Check, X, Building2, ExternalLink, Zap, Video, Eye } from 'lucide-react';
import axios from '../../api/axiosInstance';
import { toast } from 'react-toastify';

const MentorFundingRequests = () => {
    ...
    const fetchRequests = async () => {
        try {
            ...
            if (response.data.success) {
                ...
                const fundingRequests = response.data.data.filter(req => req.requestType === 'Funding');
    ...
    return (
        <div className="space-y-8 pb-20">
            <div className="flex justify-between items-end">
                <div>
                    <h2 className="text-3xl font-bold text-[var(--text-primary)]">Funding Requests</h2>
                    <p className="text-[var(--text-secondary)] mt-1">Startups seeking funding opportunities.</p>
                </div>
                    ...
                    {requests.length > 0 ? (
                            ...
                            >
                                ...
                                <div className="relative z-10">
                                        ...
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 text-xs mt-1">
                                                    <span className="px-2.5 py-1 rounded-lg font-medium border bg-green-500/10 text-green-600 border-green-500/20">
                                                        Funding
                                                    </span>
                                                    {req.startupId && (
                                    ...
                                    <div className="bg-[var(--bg-secondary)]/50 rounded-2xl p-5 mb-5 border border-[var(--text-primary)]/5">
                                        ...
                                        <p className="text-sm text-[var(--text-primary)] leading-relaxed line-clamp-2">"{req.message || "No message provided."}"</p>
                                    </div>
                        ...
                        >
                            <div className="w-24 h-24 bg-[var(--text-primary)]/5 rounded-full flex items-center justify-center mx-auto mb-6">
                                <Zap size={40} className="text-[var(--text-secondary)] opacity-50" />
                            </div>
                            <h3 className="text-2xl font-bold text-[var(--text-primary)] mb-2">All Caught Up!</h3>
                            <p className="text-[var(--text-secondary)] text-lg">You have no pending funding requests at the moment.</p>
                        </motion.div>
                    )}
                </AnimatePresence>
            </div>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                        ...
                        >
                            <div className="flex justify-between items-center mb-6 border-b border-[var(--text-primary)]/10 pb-4">
                                <div>
                                    <h2 className="text-2xl font-bold text-[var(--text-primary)]">Funding Request Details</h2>
                            ...
                            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-6">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="bg-[var(--bg-secondary)]/50 p-4 rounded-xl">
                                        <h4 className="text-xs font-bold text-[var(--text-secondary)] uppercase mb-2">Start Description & Goals</h4>
                        ...
                        >
                            <div className="text-center mb-6">
                                ...
                                <h3 className="text-2xl font-bold text-[var(--text-primary)] mb-2">
                                    {confirmModal.type === 'accept' ? 'Accept Funding Request?' : 'Decline Funding Request?'}
                                </h3>
                                <p className="text-[var(--text-secondary)]">
                                    Are you sure you want to {confirmModal.type === 'accept' ? 'accept' : 'decline'} the funding request from <span className="font-bold text-[var(--text-primary)]">{confirmModal.founderName}</span>?
                                </p>
                            </div>
...
export default MentorFundingRequests;
