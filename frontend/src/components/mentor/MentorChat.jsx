...
// Reusing same logic as DashboardChat basically, but structured for Mentor
const MentorChat = () => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (activeChat && activeChat._id) {
      ...
      const interval = setInterval(() => fetchMessages(activeChat._id), 3000); // Polling
  ...
  const fetchContacts = async (query = '') => {
    try {
      ...
      if (response.data.success && response.data.contacts) {
        // Add simulated status for demo purposes
  ...
  return (
    <div className="h-[calc(100vh-160px)] flex gap-6 relative">
        ...
        <div className="flex-1 overflow-y-auto">
          {contacts.map((contact) => (
            <div
              key={contact._id}
              onClick={() => {
                console.log("Setting active chat:", contact); // Debug Log
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-baseline mb-1">
                  <h4 className="font-semibold text-[var(--text-primary)] text-sm">{contact.name}</h4>
                </div>
      ...
      {activeChat ? (
        ...
        >
          {/* Header */}
          <div className="p-4 border-b border-[var(--text-primary)]/10 flex justify-between items-center bg-[var(--bg-primary)]/80">
            <div className="flex items-center gap-3">
              ...
              <div
                className="w-10 h-10 rounded-full bg-[var(--color-primary)]/10 flex items-center justify-center text-[var(--color-primary)] font-bold overflow-hidden cursor-pointer"
              ...
              <div className="cursor-pointer" onClick={() => setShowProfileModal(true)}>
                <h3 className="font-bold text-[var(--text-primary)]">{activeChat.name}</h3>
                <p className={`text-xs flex items-center gap-1 ${activeChat.status === 'online' ? 'text-green-500' : 'text-[var(--text-secondary)]'}`}>
          ...
          {/* Messages */}
          <div className="flex-1 p-6 overflow-y-auto space-y-4 bg-[var(--bg-primary)]/30">
            {messages.map((msg, i) => {
              // Get sender ID (handle both populated and non-populated)
              const senderId = msg.sender?._id || msg.sender;
              const isMe = senderId === user._id;

              return (
                <div key={i} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                  <div className={`p-3 rounded-2xl max-w-[70%] ${isMe ? 'bg-[var(--color-primary)] text-white rounded-tr-sm' : 'bg-[var(--text-primary)]/10 text-[var(--text-primary)] rounded-tl-sm'}`}>
          ...
          <div className="p-4 border-t border-[var(--text-primary)]/10 bg-[var(--bg-primary)]/80">
            {activeChat.mentorshipStatus === 'ended' ? (
              dismissedRequest ? (
                <div className="bg-[var(--bg-secondary)] border border-[var(--text-primary)]/10 rounded-xl p-4 flex justify-between items-center">
                  <div className="flex items-center gap-3 text-[var(--text-secondary)]">
                    <AlertCircle size={20} />
                    <span className="text-sm font-medium">Mentorship with this user has ended.</span>
                  </div>
                  <button
                    onClick={() => setDismissedRequest(false)}
                ...
                <div className="bg-red-500/10 border border-red-500/20 rounded-2xl p-6 text-center">
                  ...
                  <p className="text-[var(--text-secondary)] mb-6">
                    You have ended the mentorship with this user. You can no longer send messages.
                    <br />
                  ...
                  <div className="flex justify-center gap-4">
                    ...
                    <button
                      ...
                      className="px-6 py-2 border border-[var(--text-primary)]/10 rounded-xl text-[var(--text-secondary)] font-medium hover:bg-[var(--text-primary)]/5 transition-colors"
                    >
                      No
                    </button>
                  </div>
